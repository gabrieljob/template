...
const Page = () => {
  ...
  const handleItemDelete = useCallback((index) => {
    if (formik.values.items.length > 1) {
      formik.setFieldValue('items', formik.values.items.filter((item, _index) => _index !== index));
    }
  }, [formik]);

  const getItemFieldError = useCallback((index, field) => {
    if (!!formik.touched.items?.[index] && !!formik.errors.items?.[index]) {
      return formik.errors.items[index][field];
    }
  }, [formik]);

  const totalPrice = formik.values.items.reduce((acc, item) => acc + (item.price * item.quantity),
    0);
